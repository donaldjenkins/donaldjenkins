...
var HTMLMediaProvider = class {
  ...
  get playsinline() {
    return this.f.hasAttribute("playsinline");
  }
  set playsinline(playsinline) {
    setAttribute(this.f, "playsinline", playsinline);
  }
  get playbackRate() {
    return this.f.playbackRate;
  }
  set playbackRate(rate) {
    this.f.playbackRate = rate;
  }
  async play() {
    return this.f.play();
  }
  ...
  async loadSource({ src }, preload) {
    this.f.preload = preload;
    if (isMediaStream(src)) {
      this.f.srcObject = src;
    } else {
      this.f.srcObject = null;
      this.f.src = isString(src) ? src : window.URL.createObjectURL(src);
    }
    this.f.load();
  }
};
