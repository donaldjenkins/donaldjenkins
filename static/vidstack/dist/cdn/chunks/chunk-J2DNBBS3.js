import { VideoProvider, TextTrack, TEXT_TRACK_READY_STATE, TEXT_TRACK_ON_MODE_CHANGE } from './chunk-TA667RH3.js';
import { isHLSSupported, IS_CHROME, createRAFLoop, LIST_SELECT, LIST_ADD } from './chunk-XUPMJMXW.js';
import { dispatchEvent, DOMEvent, deferredPromise, camelToKebabCase } from './chunk-FFBW6SEG.js';
import { isNull, signal, isString, effect2, peek, isUndefined, isFunction2 } from './chunk-GKRV3IQ4.js';
...
function setupHLS(provider, { player, logger, delegate, $store, qualities, audioTracks, textTracks }, callbacks) {
  ...
  function onError(eventType, data) {
    {
      logger?.errorGroup(`HLS error \`${eventType}\``).labelledLog("Media Element", provider.instance?.media).labelledLog("HLS Instance", provider.instance).labelledLog("Event Type", eventType).labelledLog("Data", data).labelledLog("Src", $store.source).labelledLog("Media Store", { ...$store }).dispatch();
    }
    if (data.fatal) {
      switch (data.type) {
        case "networkError":
...
export { ENABLE_AUTO_QUALITY, HLSProvider, HLS_PROVIDER, SET_AUTO_QUALITY, coerceToError, preconnect };
