...
/* Cloudflare's box ends with two adjacent links -- "Cloudflare" and
   "Privacy" -- separated by nothing. Its own cf.errors.css would space them,
   but that stylesheet is not loaded on a custom page, so they render as
   "CloudflarePrivacy". Confirmed in the dashboard preview, 23 Aug 2026. */
.cf-box a + a { margin-left: 0.6em; }
